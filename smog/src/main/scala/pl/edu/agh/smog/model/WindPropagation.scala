...
object WindPropagation {
  def calculateSmellAddends(smogConfig: SmogConfig): (CellArray, Int, Int) => Vector[Option[Signal]] = {
    def calculateSmellAddends(cells: CellArray, x: Int, y: Int): Vector[Option[Signal]] = {
      ...
      val crossWeight: Double = smogConfig.crossBendFactor
      val straightWeight: Double = smogConfig.straightBendFactor
