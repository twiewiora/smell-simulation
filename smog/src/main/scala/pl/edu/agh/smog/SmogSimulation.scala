...
class SmogSimulation[ConfigType <: XinukConfig : ValueReader](
                                                           configPrefix: String,
                                                           metricHeaders: Vector[String],
                                                           conflictResolver: ConflictResolver[ConfigType],
                                                           smellPropagationFunction: SmogConfig => (CellArray, Int, Int) => Vector[Option[Signal]],
  ...
  private val workerRegionRef: ActorRef =
    ClusterSharding(system).start(
      ...
      entityProps = WorkerActor.props[ConfigType](workerRegionRef, movesControllerFactory, conflictResolver, smellPropagationFunction(config.asInstanceOf[SmogConfig]), emptyCellFactory),
