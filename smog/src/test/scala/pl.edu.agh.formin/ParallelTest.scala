...
import pl.edu.agh.smog.model.{AlgaeAccessible, AlgaeCell, ForaminiferaAccessible, ForaminiferaCell}
...
class ParallelTest extends FlatSpec with Matchers with Eventually with ScalaFutures {
  ...
  "A WorkerActors" should "have defined buffers correctly for worker surrounded by neighbours" in new Fixture {
    ...
    val worker = TestActorRef(WorkerActor.props[SmogConfig](workerRegion.ref, (bufferZone, config) =>
    ...
    val grid: Grid = worker.underlyingActor.asInstanceOf[WorkerActor[SmogConfig]].grid
  ...
  it should "should make cells migrations correctly" in new Fixture {
    val workerRegion = TestProbe("workerRegion")
    val worker1 = TestActorRef(WorkerActor.props[SmogConfig](workerRegion.ref, (bufferZone, config) =>
      new SmogMovesController(bufferZone)(config), SmogConflictResolver,
      DefaultSmellPropagation.calculateSmellAddendsStandard
    ))
    val worker2 = TestActorRef(WorkerActor.props[SmogConfig](workerRegion.ref, (bufferZone, config) =>
      new SmogMovesController(bufferZone)(config), SmogConflictResolver,
      DefaultSmellPropagation.calculateSmellAddendsStandard
    ))
    ...
    val workers1Grid: Grid = worker1.underlyingActor.asInstanceOf[WorkerActor[SmogConfig]].grid
    ...
    val workers2Grid: Grid = worker2.underlyingActor.asInstanceOf[WorkerActor[SmogConfig]].grid
